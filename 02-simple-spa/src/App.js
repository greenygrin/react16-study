...
// route 기능을 위한 패키지에서 Route 함수와 Link 함수만 골라서 참조
...
const App = () => {
    return (
        <div>
            ...
            <Link to="/department_get?deptno=101&msg=hello">
                [컴퓨터공학과]
            </Link>
            <Link to="/department_get?deptno=102&msg=world">
                [멀티미디어학과]
            </Link>
            ...
            <Switch>
                {" "}
                {/* 404 error를 사용할 때는 반드시 switch 태그로 감싸야 함 */}
                {/* 첫 페이지로 사용되는 컴포넌트의 경우 exact={true}를 명시해야 한다  */}
                <Route path="/" component={Home} exact={true} />
                {/* 한 페이지에 두개 이상의 URL을 적용할 경우 배열 형식으로 처리함 */}
