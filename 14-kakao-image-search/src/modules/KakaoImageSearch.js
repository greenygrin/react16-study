...
/**
 * 5) 비동기 작업을 수행할 함수 정의
 * 컨테이너로 부터 query라는 파라미터를 전달받는다. (파라미터는 개발자가 상황에 따라 정의함)
...
export const imageSearchAsync = query => async dispatch => {
    ...
    try {
        const result = await axios.get('https://dapi.kakao.com/v2/search/image', {
            ...
            headers: {
                Authorization: 'KakaoAK 797bcc2ed1e00f75ef5de16df9678988'
            }
        });
        ...
        // console.error(e);
        // dispatch(searchFailureAction({ error: '검색결과를 가져오는데 실패했습니다.' }));
        
        // kakao에서 보내주는 에러메세지 전문 받기
        // --> {'errorType': 'AccessDeniedError', 'message': 'cannot fine appkey'}
