 ...
 * 해당 컨테이너 컴포넌트의 부모 컴포넌트가 리렌더링될 때 해당 컨테이너 컴포넌트의 props가 바뀌지 않았다면 리렌더링이 자동으로 방지되어 성능이 최적화되지만,
 * 
 * 반면 useSelector를 사용하여 리덕스 상태를 조회했을 때는 (hook을 사용할 경우)
 * 이 최적화 작업이 자동으로 이루어지지 않으므로 성능 최적화를 위해서는 React.memo를 컨테이너 컴포넌트에 사용해 주어야 한다.
 * 
 * 이 컨테이너는 CounterModule을 ContainerCounter와 공유하고 있기 때문에 상태값과 액션 함수도 공유된다. --> Redux를 사용하는 이유
 */
